        ...
        public void UpdateLayerInfo()
        {
            for (int i = 0; i < dataGridView3.Rows.Count; i++)
            {
                ...
                if (i < LaserProject.Model3.Layers.Count)
                {
                    ...
                    LaserProject.Model3.Layers[i].LaserPower = yjCommon.StrToDoubleDef(row.Cells[5].Value?.ToString(), 0.0);
                    LaserProject.Model3.Layers[i].ZOffset = yjCommon.StrToDoubleDef(row.Cells[6].Value?.ToString(), 0.0);
                    LaserProject.Model3.Layers[i].PulsePitch = yjCommon.StrToDoubleDef(row.Cells[7].Value?.ToString(), 0.0);
                }
            }
        }
